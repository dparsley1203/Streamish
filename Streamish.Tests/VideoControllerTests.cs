...
namespace Streamish.Tests
{
    public class VideoControllerTests
    {
        ...
        {
            // Arrange 
            var videoCount = 20; //sets videocount to 20
            var videos = CreateTestVideos(videoCount); //creates 20 video objects

            var repo = new InMemoryVideoRepository(videos); //passes in 20 video objects into a copy of video repo that lives in memory
            var controller = new VideoController(repo); //creates a new controller instance 

            // Act 
            var result = controller.Get(); //calls the get method in the test/copy of VideoController
